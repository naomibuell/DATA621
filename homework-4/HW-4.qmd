...
The insurance training data set contains `r nrows` rows and `r ncols` features, and is `r perc_complete` percent complete. 
...
tbl_summ_stats <- insurance_train |>
    skim_without_charts() |>
    janitor::clean_names() |>
    as_tibble() |>
...
The target variables are `TARGET_FLAG`, which represents whether the person was in a car crash (1) or not (0), and `TARGET_AMT`, which represents the cost of the crash if it occurs. The other variables represent various information on auto insurance for different individuals. There are `r  @tbl-summary-statistics presents descriptive characteristics of the training data.
...
insurance_train_cor <- insurance_train |>
    cor(use = "pairwise.complete.obs")

corrplot(insurance_train_cor, method = "circle", type = "upper", tl.cex = .6, sig.level = 0.05, insig = "blank", addCoef.col = "black", number.cex = .5)

most_corr_w_target <- insurance_train_cor |>
    as.data.frame() |>
