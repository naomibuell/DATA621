...
In conclusion, our selected models demonstrate strong predictive capabilities for insurance outcomes:

- **Logistic Model** (`r best_logistic_name`): Achieved an accuracy of `r round(best_logistic$Accuracy, 3)` and AUC of `r round(best_logistic$AUC, 3)` for predicting crash probability.

- **Linear Model** (`r best_linear_name`): Achieved an R-squared of `r round(best_linear$R_Squared, 3)` for predicting claim costs.

The significant predictors identified in both models provide valuable insights into the key factors driving insurance claims. These findings can inform risk assessment and pricing strategies for the insurance company.