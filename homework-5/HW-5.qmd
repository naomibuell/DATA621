...
wine_train_prep |>
  ...
  select(log_target, all_of(key_vars)) |>
...
We follow a workflow centered on building valid models and assessing predictive performance, as emphasized in our primary texts:

Partitioning: Following AMAR Section 7.3, we split data into a training set for model development and a test set for assessing predictive ability. LMR Section 9.1 also notes the use of a validation sample to avoid over-estimation of model quality.

Poisson GLM: While LMR Section 1.3 identifies Poisson regression as a key extension for count responses, it primarily focuses on Gaussian linear models. We use the Pearson dispersion check to verify the model's validity.

Lasso Penalization: We implement LASSO as a modern development for variable selection
