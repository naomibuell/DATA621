...
# Remove INDEX variable (if present)
wine_train_prep <- wine_train |> clean_names() |> select(-any_of("index"))
wine_eval_prep  <- wine_eval  |> clean_names() |> select(-any_of("index"))
...
wine_train_prep <- wine_train_prep |>
    mutate(
        ...
        log_residualsugar = log1p(residualsugar),
        log_freesulfurdioxide = log1p(freesulfurdioxide),
        log_totalsulfurdioxide = log1p(totalsulfurdioxide),

        sulfur_ratio = totalsulfurdioxide / (freesulfurdioxide + 1),
        acidity_ratio = volatileacidity / (fixedacidity + 1),
        density_alcohol_ratio = density / (alcohol + 1),
        quality_score = stars * labelappeal,
...
```{r}
#| label: split-wine-data
#| eval: true
# Prepare train/test splits (80/20) and detect the target column robustly
set.seed(19940211)

# detect target column (case-insensitive 'target')
target_var <- names(wine_train_prep)[tolower(names(wine_train_prep)) == "target"]
if (length(target_var) == 0) stop("No 'target' column found in wine_train_prep")
target_var <- target_var[1]

train_idx <- createDataPartition(wine_train_prep[[target_var]], p = 0.8, list = FALSE)
wine_train_split <- wine_train_prep[train_idx, ]
wine_test_split  <- wine_train_prep[-train_idx, ]

# Use numeric predictors by default (safe for GLMs)
numeric_cols <- names(wine_train_split)[vapply(wine_train_split, is.numeric, logical(1))]
predictors <- setdiff(numeric_cols, target_var)
formula_all <- reformulate(predictors, response = target_var)
cat("Using target:", target_var, "and", length(predictors), "numeric predictors.\n")
```

```{r}
#| label: model-poisson
cat("--- Poisson GLM (baseline) ---\n")
poisson_mod <- glm(formula_all, data = wine_train_split, family = poisson(link = "log"))
poisson_disp <- sum(residuals(poisson_mod, type = "pearson")^2) / df.residual(poisson_mod)
cat("Poisson dispersion:", signif(poisson_disp, 4), "\n")
```

```{r}
#| label: model-nb
if (requireNamespace("MASS", quietly = TRUE)) {
    cat("--- Negative Binomial (MASS::glm.nb) ---\n")
    nb_mod <- tryCatch(MASS::glm.nb(formula_all, data = wine_train_split), error = function(e) { message("NB fit failed: ", e$message); NULL })
} else cat("MASS not available; skipping Negative Binomial.\n")
```

```{r}
#| label: model-zip
if (requireNamespace("pscl", quietly = TRUE)) {
    cat("--- Zero-inflated Poisson (pscl::zeroinfl) ---\n")
    zip_mod <- tryCatch(
        pscl::zeroinfl(as.formula(paste(target_var, "~", paste(predictors, collapse = "+"))), data = wine_train_split, dist = "poisson"),
        error = function(e) { message("ZIP fit failed: ", e$message); NULL }
    )
} else cat("pscl not available; skipping ZIP/ZINB models.\n")
```

```{r}
#| label: model-glmnet
if (requireNamespace("glmnet", quietly = TRUE)) {
    cat("--- Penalized Poisson (glmnet) ---\n")
    x_train <- model.matrix(formula_all, wine_train_split)[, -1, drop = FALSE]
    y_train <- wine_train_split[[target_var]]
    cv_glmnet_lasso <- glmnet::cv.glmnet(x_train, y_train, family = "poisson", alpha = 1)
    glmnet_mod <- cv_glmnet_lasso
    cat("glmnet: lambda.min =", signif(glmnet_mod$lambda.min, 4), "\n")
} else cat("glmnet not installed; skipping penalized models.\n")
```

```{r}
#| label: model-xgboost
if (requireNamespace("xgboost", quietly = TRUE)) {
    cat("--- xgboost (count:poisson) ---\n")
    x_train <- model.matrix(formula_all, wine_train_split)[, -1, drop = FALSE]
    dtrain <- xgboost::xgb.DMatrix(data = x_train, label = wine_train_split[[target_var]])
    params <- list(objective = "count:poisson", eval_metric = "rmse")
    xgb_mod <- tryCatch(xgboost::xgb.train(params = params, data = dtrain, nrounds = 100, verbose = 0), error = function(e) { message("xgboost training failed: ", e$message); NULL })
} else cat("xgboost not installed; skipping.\n")
```
