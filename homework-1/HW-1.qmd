...
::: callout-note
Comment from Alex: I think we should add box plots or some other way to identify outliers (eg, Z scores). It can be hard to see outliers in histograms. If there are bad leverage points in our models, we may need to create dummy variables for the outliers.
:::
...
::: callout-note
Comment from Alex: Can we plot the histograms below separately and then combine them? Currently, all 3 histograms use the default binwidth, which may not apply to every plot (the middle one in particular needs a different value). Alternatively, we can consider a density histogram that doesn't use binwidth.
:::
