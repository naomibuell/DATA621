...
header-includes:
  ...
  - \pagestyle{fancy}
  - \lhead{DATA 621}
  - \rhead{Naomi Buell, Richie Rivera,Alexander Simon and Nana Frimpong}
...
To begin by remove variables with high missinng  vairables and low correlation with  `TARGET_WINS`: `TEAM_BATTING_HBP` (91.6% missing), `TEAM_BASERUN_CS` (33.9% missing),  and `TEAM_FIELDING_DP` (12.6% missing).
...
# Drop high missign vrianbels and low-correlation variables with the datat
# these variables lack a solid relationship with response variable TARGET_WINS
moneyball_clean <- moneyball_train |>
  select(-TEAM_BATTING_HBP, -TEAM_BASERUN_CS, -TEAM_FIELDING_DP)
```

Before our predictor variables `TEAM_BASERUN_CS`, `TEAM_BASERUN_SB` and `TEAM_PITCHING_SO` are imputed, a binary indicator  is created to preserve infomation  that value was missing in first place. Our indicator for each flag follows (1 =  missing 0 = present)

```{r}
#| label: missingness flags inddicator
#| echo: true
#| warning: false
...
Imputation now occurs based on the distribution of missing data of the following variables  `TEAM_BASERUN_SB`, `TEAM_PITCHING_SO` , and `TEAM_BATTING_SO`. `TEAM_BASERUN_SB` and `TEAM_PITCHING_SO` are right skewed therefore it is imputated around the mean and `TEAM_BATTING_SO` is bimoal , so we can imputate around with the media to avoid placing the imputated values in region between two peak where only few observation exist. 
```{r}
#| label: imputation
#| echo: true
#| warning: false
...
moneyball_clean <- moneyball_clean %>%
  mutate(
    ...
    # Median imputation for bimodal variable distribution
...
When a distribution is observe ot be skewed to th right, meaning small number of outliers has influence on the response variable `TARGET_WINS`, the log transformation can be applied to balance the scale of distribution. Utlizing teh log compress the distribution of missing variable closer to symmetic shape. This makes the relationship with `TARGET_WINS` more lienar and easier for the model to capture. Log1p is used to handle any zero without producing undefined resutls
...
We can create three new variables that captures negative and postive impact on the wins for the team. 
...
```
 ...
 Finally we can verify that there no missing variables taht remain based on teh results of our impuation 
...
# Verify no remaining missingness
moneyball_clean |>
